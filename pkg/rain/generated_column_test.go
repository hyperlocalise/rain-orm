...
func TestGeneratedColumnNotNull(t *testing.T) {
	...
	// Generated Always clause MUST come after NOT NULL in Postgres for consistency with Identity.
...
func TestPostgresIdentityColumns(t *testing.T) {
	t.Parallel()

	type IdentityTable struct {
		schema.TableModel
		ID      *schema.Column[int64]
		OtherID *schema.Column[int32]
	}

	table := schema.Define("identity_test", func(t *IdentityTable) {
		t.ID = t.BigSerial("id").GeneratedAlwaysAsIdentity().PrimaryKey()
		t.OtherID = t.Serial("other_id").GeneratedByDefaultAsIdentity()
	})
	...
	// BigSerial should be downgraded to BIGINT, Serial to INTEGER.
	// NOT NULL must come before GENERATED ... AS IDENTITY.
	expectedID := `"id" BIGINT PRIMARY KEY NOT NULL GENERATED ALWAYS AS IDENTITY`
	expectedOther := `"other_id" INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY`

	if !strings.Contains(sql, expectedID) {
		t.Errorf("expected ID identity clause, got:\n%s", sql)
	}
	if !strings.Contains(sql, expectedOther) {
		t.Errorf("expected OtherID identity clause, got:\n%s", sql)
	}
}

func TestIdentityColumnValidation(t *testing.T) {
	t.Parallel()

	dbMySQL, _ := rain.OpenDialect("mysql")
	dbPostgres, _ := rain.OpenDialect("postgres")

	t.Run("Identity on MySQL error", func(t *testing.T) {
		table := schema.Define("t", func(t *struct {
			schema.TableModel
			ID *schema.Column[int64]
		},
		) {
			t.ID = t.BigInt("id").GeneratedAlwaysAsIdentity()
		})
		_, err := dbMySQL.CreateTableSQL(table)
		if err == nil || !strings.Contains(err.Error(), "identity columns are only supported by postgres dialect") {
			t.Fatalf("expected error for MySQL identity, got: %v", err)
		}
	})

	t.Run("Identity with Default error", func(t *testing.T) {
		table := schema.Define("t", func(t *struct {
			schema.TableModel
			ID *schema.Column[int64]
		},
		) {
			t.ID = t.BigInt("id").GeneratedAlwaysAsIdentity().Default(10)
		})
		_, err := dbPostgres.CreateTableSQL(table)
		if err == nil || !strings.Contains(err.Error(), "cannot have a DEFAULT clause") {
			t.Fatalf("expected error for identity with default, got: %v", err)
		}
	})
}
