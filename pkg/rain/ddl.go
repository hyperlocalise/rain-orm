...
func columnDefinitionSQL(d dialect.Dialect, table *schema.TableDef, column *schema.ColumnDef, inlinePrimaryKey bool) (string, error) {
	...
	if column.Identity != schema.IdentityNone {
		if d.Name() != "postgres" {
			return "", fmt.Errorf("rain: identity columns are only supported in postgres dialect, got %s", d.Name())
		}
		if column.HasDefault || column.DefaultSQL != "" || column.DefaultExpr != nil {
			return "", fmt.Errorf("rain: identity column %q cannot have a DEFAULT clause", column.Name)
		}
		if column.GeneratedExpr != nil {
			return "", fmt.Errorf("rain: identity column %q cannot have a GENERATED AS (...) clause", column.Name)
		}
	}
	...
	if !column.Nullable && (!inlinePrimaryKey || (d.Name() == "postgres" && column.Identity != schema.IdentityNone)) {
