...
func columnDefinitionSQL(d dialect.Dialect, table *schema.TableDef, column *schema.ColumnDef, inlinePrimaryKey bool) (string, error) {
	...
	if column.Identity != schema.IdentityNone && d.Name() == "postgres" {
		if !column.Nullable && !inlinePrimaryKey {
			parts = append(parts, "NOT NULL")
		}
		if column.HasDefault || column.DefaultSQL != "" || column.DefaultExpr != nil {
			return "", fmt.Errorf("rain: identity column %q cannot have a DEFAULT value", column.Name)
		}
		if column.GeneratedExpr != nil {
			return "", fmt.Errorf("rain: identity column %q cannot also be a non-identity generated column", column.Name)
		}
	...
	} else if column.AutoIncrement && shouldEmitAutoIncrementKeyword(d, column, inlinePrimaryKey) {
		parts = append(parts, d.AutoIncrementKeyword())
	}
	if column.GeneratedExpr != nil && (column.Identity == schema.IdentityNone || d.Name() != "postgres") {
	...
	if !column.Nullable && !inlinePrimaryKey && (column.Identity == schema.IdentityNone || d.Name() != "postgres") {
		parts = append(parts, "NOT NULL")
	}
	...
	if (column.HasDefault || column.DefaultSQL != "" || column.DefaultExpr != nil) && (column.Identity == schema.IdentityNone || d.Name() != "postgres") {
