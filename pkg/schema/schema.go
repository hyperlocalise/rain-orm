...
// IdentityType describes PostgreSQL identity column semantics.
...
// GeneratedAlwaysAsIdentity marks the column as an identity column (GENERATED ALWAYS AS IDENTITY).
...
// GeneratedByDefaultAsIdentity marks the column as an identity column (GENERATED BY DEFAULT AS IDENTITY).
// Supported by PostgreSQL.
...
func cloneTableDef(src *TableDef, alias string) *TableDef {
	...
	for _, column := range src.Columns {
		...
		copyColumn.Identity = column.Identity
